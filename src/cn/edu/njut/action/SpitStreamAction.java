...
public class SpitStreamAction {
	...
	public void execute() throws Exception {
		...
		if(ctid == null) {
			...
			ctidSet += ")";
			@SuppressWarnings("unchecked")
			List<Spit> temp = (List<Spit>)dao.getByHql("FROM Spit s WHERE s.ctid in " + ctidSet + " AND s.content!=null AND s.content!='' AND s.content!=' ' ORDER BY s.spitid DESC");
			List<Spit> list = new ArrayList<Spit>();

			if(lastupdate == null) {
				int lastupdate = 0;
				for(int i = 0; i<temp.size() && i<20; i++) {
					list.add(temp.get(i));
					lastupdate = temp.get(i).getSpitid();
				}
				InfoStream infostream = new InfoStream(list);
				String json = infostream.getInfoStreamJson("Spit");
			...
			if(lastupdate != null) {
				...
				for(int i = 0; i<temp.size() && count<20; i++) {
					if(temp.get(i).getSpitid()<lastupdate) {
						list.add(temp.get(i));
						count++;
						lastup = temp.get(i).getSpitid();
					}
				}
				InfoStream infostream = new InfoStream(list);
				String json = infostream.getInfoStreamJson("Spit");
		...
		if(lastupdate == null) {
			@SuppressWarnings("unchecked")
			List<Spit> temp = (List<Spit>)dao.getByHql("FROM Spit s WHERE s.ctid=" + ctid + " AND s.content!=null AND s.content!='' AND s.content!=' ' ORDER BY s.spitid DESC");
			List<Spit> list = new ArrayList<Spit>();
			int lastupdate = 0;
			for(int i = 0; i<temp.size() && i<20; i++) {
				list.add(temp.get(i));
