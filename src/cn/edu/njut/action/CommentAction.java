...
public class CommentAction {
	...
	public void execute() throws Exception {
		...
		content = temp;
		if(tag == 1) {	// comment
			Comment c = new Comment(edid, from, to, content, "[S]");
			dao.save(c);
			Notification n = new Notification(edid, from, to, "[S]you get a COMMENT");
			dao.save(n);
			writer.write("{\"msg\":\"succeed\"}");
			writer.flush();
			NotifyService.pushSingleAccount(to, "new COMMENT", "[S,SPIT,"+ edid + "]" + notifyContent);
			return;
		}
		...
		if(tag == 0) {	// cancel
			...
			dao.delete(c);
			Notification n = (Notification)dao.getOne("FROM Notification n WHERE n.content='[S]you get a COMMENT' AND n.infoid=" + edid + " AND n.fromuser='" + from + "' AND n.touser='" + to + "'");
			if(n != null)
				dao.delete(n);
