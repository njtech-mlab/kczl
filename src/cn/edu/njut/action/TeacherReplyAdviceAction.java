...
public class TeacherReplyAdviceAction {
	...
	public void execute() throws Exception {
		...
		if(tag == 1) {	// comment
			...
			content = "[T]" + content;
			Advicecomment c = new Advicecomment(edid, from, to, content, "[T]");
			dao.save(c);
			Notification n = new Notification(edid, from, to, "[T]you get a COMMENT");
			dao.save(n);
			writer.write("{\"msg\":\"succeed\"}");
			writer.flush();
			NotifyService.pushSingleAccount(to, "new COMMENT", "[T,ADVICE,"+ edid + "]" + notifyContent);
			return;
		}
